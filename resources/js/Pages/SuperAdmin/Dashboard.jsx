...
export default function Dashboard({ metrics, recentAccounts, recentForms, isSuperAdmin, deploymentData }) {
        ...
        const rawRole = auth?.user?.role?.toLowerCase()?.trim() || '';
        const isFeedbackCommittee = rawRole === 'feedback committee' || rawRole === 'feedbackcommittee';
        ...
        // Count active filters (SuperGroup checks if dept !== overall)
        const activeFilterCount = (isSuperGroup && currentDept !== 'overall' ? 1 : 0) + 
        ...
        const handleApplyFilters = () => {
            const params = {};
            if (isSuperGroup && tempDept !== 'overall') params.department = tempDept;
            ...
            const currentRoute = isSuperGroup ? 'superadmin.dashboard' : 'focalperson.dashboard';
        ...
        const focalDeptName = !isSuperGroup && deptArray.length > 0 
        ...
        const filteredTrendData = useMemo(() => {
            ...
            if (!showLocalTabs) return trendData;
        ...
        // SuperAdmin sees CC. Focal person sees CC if they have responses. Feedback Committee NEVER sees CC.
       const showCC = isSuperAdmin || (!isSuperGroup && ccMetrics.total_responses > 0);
    ...
    return (
        <SuperAdminLayout headerTitle={dashboardTitle}>
            ...
            <div className="max-w-7xl mx-auto space-y-6 pb-10">
                        ...
                        <div className="relative w-full sm:w-auto" ref={filterRef}>
                            ...
                            {filterOpen && (
                                <div className="absolute left-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 p-4 z-50 space-y-4">
                                    ...
                                    {/* Both SuperAdmin and Feedback Committee can filter by department */}
                                    {isSuperGroup && (
                ...
                <div className={`grid grid-cols-1 md:grid-cols-2 ${isSuperAdmin ? 'lg:grid-cols-3 xl:grid-cols-6' : isFeedbackCommittee ? 'lg:grid-cols-5' : 'lg:grid-cols-2'} gap-4`}>
                    
                    {/* --- FEEDBACK COMMITTEE AUDIT KPIs --- */}
                    {isFeedbackCommittee && (
                        <>
                            {/* 1. Audited Feedback */}
                            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Audited Feedback</p>
                                        <p className="text-2xl font-bold text-gray-900 mt-0.5">{metrics?.total_audited_feedback || 0}</p>
                                    </div>
                                    <div className="bg-orange-100 p-2 rounded-lg text-orange-600 text-base">
                                        <i className="fa-solid fa-comments"></i>
                                    </div>
                                </div>
                                <div className="mt-2 text-xs text-gray-400 font-medium">From compliant departments</div>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition">
                                ...
                                <div className="mt-2 text-xs text-gray-400 font-medium">Total integrated</div>
                            </div>
                            ...
                            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Report Submissions</p>
                                        <p className="text-2xl font-bold text-gray-900 mt-0.5">
                                            {metrics?.compliant_departments || 0} <span className="text-gray-400 text-lg">/ {metrics?.total_departments || 0}</span>
                                        </p>
                                    </div>
                                    <div className="bg-emerald-100 p-2 rounded-lg text-emerald-600 text-base">
                                        <i className="fa-solid fa-file-circle-check"></i>
                                    </div>
                                </div>
                                <div className="mt-2 text-xs text-gray-400 font-medium">
                                    <span className="font-semibold text-gray-600">{metrics?.pending_reports || 0}</span> pending review
                                </div>
                            </div>

                            {/* 5. Flagged Harassment */}
                            <div className={`bg-white p-4 rounded-lg shadow-sm border-2 flex flex-col justify-between hover:shadow-md transition ${(metrics?.audited_harassment || 0) > 0 ? 'border-red-500' : 'border-gray-200'}`}>
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className={`text-xs font-semibold uppercase tracking-wide ${(metrics?.audited_harassment || 0) > 0 ? 'text-red-600' : 'text-gray-500'}`}>Flagged Harassment</p>
                                        <p className="text-2xl font-bold text-gray-900 mt-0.5">{metrics?.audited_harassment || 0}</p>
                                    </div>
                                    <div className={`p-2 rounded-lg text-base ${(metrics?.audited_harassment || 0) > 0 ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-400'}`}>
                                        <i className="fa-solid fa-triangle-exclamation"></i>
                                    </div>
                                </div>
                                <div className="mt-2 text-xs text-gray-400 font-medium">Escalated via reports</div>
                            </div>
                        </>
                    )}

                    {/* --- SUPERADMIN & FOCAL PERSON KPIs --- */}
                    {!isFeedbackCommittee && (
                        <>
                            {/* 1. Total Feedback */}
                            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition">
                            ...
                            {/* STRICTLY SuperAdmin Only (KPIs 2-5) */}
                            {isSuperAdmin && (
                                <>
                                    {/* 2. Active Forms */}
                                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition">
                                    ...
                                    {/* 4. Report Submissions (NEW FOR SUPERADMIN) */}
                                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Report Submissions</p>
                                    ...
                                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between hover:shadow-md transition">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Total Accounts</p>
                                                <p className="text-2xl font-bold text-gray-900 mt-0.5">{metrics?.total_users || 0}</p>
                                            </div>
                                            <div className="bg-indigo-100 p-2 rounded-lg text-indigo-600 text-base">
                                                <i className="fa-solid fa-users-gear"></i>
                                            </div>
                                        </div>
                                        <div className="mt-2 text-xs text-gray-400 font-medium">Registered system users</div>
                                    </div>
                                </>
                            )}

                            {/* 6. Harassment Alerts (SuperAdmin & Focal Person) */}
                            <div className={`bg-white p-4 rounded-lg shadow-sm border-2 flex flex-col justify-between hover:shadow-md transition ${harassmentCount > 0 ? 'border-red-500' : 'border-gray-200'}`}>
                                ...
                                <div className="mt-2 flex justify-between items-center text-xs font-medium">
                                    ...
                                    {harassmentCount > 0 && <span className="text-red-600 font-semibold animate-pulse ml-2">Needs review</span>}
                                </div>
                            </div>
                        </>
                    )}
                ...
                {isFeedbackCommittee && (metrics?.total_audited_feedback || 0) === 0 ? (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-16 flex flex-col items-center justify-center text-center mt-6">
                        <div className="w-24 h-24 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center mb-5 border-4 border-blue-100/50">
                            <i className="fa-solid fa-file-shield text-4xl"></i>
                        </div>
                        <h3 className="text-2xl font-bold text-gray-900 mb-2">Awaiting  Submissions</h3>
                        <p className="text-gray-500 max-w-lg mx-auto text-sm leading-relaxed">
                            No official reports have been submitted for this period yet. Analytics, compliance tracking, and demographic trends will populate here automatically once Focal Persons finalize and submit their audited feedback data.
                        </p>
                    </div>
                ) : (
                    <>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                            ...
                            {/* Department Performance Ranking - STRICTLY SUPERADMIN ONLY */}
                            {isSuperGroup && (
                                <div className="bg-white rounded-xl shadow-sm border border-gray-200/80 p-6 lg:col-span-3 transition duration-200 hover:shadow-md">
                            ...
                            {/* --- REGION OF RESIDENCE ANALYTICAL GRAPH --- */}
                            <div className="bg-white rounded-xl shadow-sm border border-gray-200/80 p-6 lg:col-span-3 transition duration-200 hover:shadow-md">
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-1">
                                            {isSuperGroup && currentDept === 'overall'
                                                ? 'Geographic distribution of respondents across all university transactions'
                                                : `Geographic distribution of respondents for ${!isSuperGroup ? focalDeptName : (deptArray.find(d => String(d.department_id) === String(currentDept))?.department_name || 'selected department')}`
                                            }
                                        </p>
                                    </div>
                                    ...
                                    <div className="w-full h-72">
                                        <ResponsiveContainer width="100%" height="100%">
                                            ...
                                            >
                                                ...
                                                <YAxis type="category" dataKey="name" tick={{ fontSize: 12, fill: '#334155', fontWeight: 500 }} axisLine={false} tickLine={false} width={140} />
                                                <RechartsTooltip 
                                                    cursor={{ fill: '#f8fafc' }}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            {/* ONLY SuperAdmin (or FocalPerson with responses) sees this. Feedback Committee NEVER sees this. */}
                            {showCC && (
                            ...
                            {/* Recurring Complaints gracefully shifts layout if CC Compliance is hidden */}
                            <div className={`bg-white rounded-xl shadow-sm border border-gray-200/80 p-6 flex flex-col justify-between transition duration-200 hover:shadow-md ${showCC ? '' : 'lg:col-span-3'}`}>
                                <div>
