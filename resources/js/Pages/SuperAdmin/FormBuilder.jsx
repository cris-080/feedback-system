...
export default function FormBuilder({ departments, departmentServices, initialFields }) {
    
    // 1. Filter out empty placeholder questions sent by the backend
    const validInitialFields = (initialFields || []).filter(
        field => field.field_label && field.field_label.trim() !== ''
    );

    // 2. Setup Form State[cite: 5]
    ...
    // 3. Dynamic Department Services Sync & Department Name Lock
    useEffect(() => {
        if (data.department_id) {
            const services = departmentServices[data.department_id] || [];
            
            // Use functional update to prevent stale closures and infinite loop bugs
            setData(currentData => {
                const updatedFields = currentData.fields.map(field => {
                    
                    // FIX 1: Force 'Service Availed' to be a dropdown populated with department services
                    if (field.field_label === 'Service Availed') {
                        const newOptions = services.length > 0 ? services : ['General Transaction'];
                        return { ...field, input_type: 'dropdown', options: newOptions };
                    }
                    
                    // FIX 2: Force 'Name of Office/Department' to be standard text, storing the name in options[0] for the viewer
    ...
    // 4. Dynamic Step Title Generator
                        ...
                        {[1, 2, 3, 4].map(step => {
                            ...
                            return (
                                <div key={`step-${step}`} className="bg-white p-6 rounded-lg border-2 border-blue-600 mb-8 shadow-sm">
                                    ...
                                    {/* NEW: Editable Step Instructions */}
                                    {step === 1 && (
                                        <div className="mb-6 bg-blue-50 border border-blue-200 p-4 rounded-md">
                                            <label className="block text-sm font-bold text-blue-800 mb-2">
                                                <i className="fa-solid fa-circle-info mr-1"></i> Step 1 Instructions (Visible to Citizen)
                                            </label>
                                            <textarea className="w-full border-blue-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm text-gray-700" rows="3"
                                                value={data.step_1_instruction} onChange={e => setData('step_1_instruction', e.target.value)}></textarea>
                                        </div>
                                    )}
                                    ...
                                    {step === 2 && data.form_type === 'CC' && (
                                        <div className="mb-6 bg-blue-50 border border-blue-200 p-4 rounded-md">
                                            <label className="block text-sm font-bold text-blue-800 mb-2">
                                                <i className="fa-solid fa-circle-info mr-1"></i> Step 2 Instructions (Visible to Citizen)
                                            </label>
                                            <textarea className="w-full border-blue-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm text-gray-700" rows="2"
                                                value={data.step_2_instruction} onChange={e => setData('step_2_instruction', e.target.value)}></textarea>
                                        </div>
                                    )}
                                    {/* End of Editable Step Instructions */}
                                    
                                    {data.fields.filter(f => f.step_number === step).map((field, index) => {
                                        const isDeptLocked = field.field_label === 'Name of Office/Department';
                                        const isServiceLocked = field.field_label === 'Service Availed';
                                            ...
                                            <div key={`field-${step}-${field.field_id}-${index}`} className={`p-5 mb-5 rounded-md border ${isLocked ? 'bg-gray-100 border-gray-300 border-l-4 border-l-gray-500' : 'bg-gray-50 border-gray-200 border-l-4 border-l-blue-500'} relative`}>
                                                ...
                                                {isLocked ? (
                                                    <div className="absolute top-4 right-4 text-gray-500 text-sm font-bold">
                                                        ...
                                                        {isServiceLocked ? 'Locked (Managed Services)' : 'Locked (Auto-synced)'}
                                                    </div>
                                                ) : (
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                                                    <div className="md:col-span-7">
                                                        ...
                                                        <input type="text" className="w-full border-gray-300 rounded-md shadow-sm disabled:bg-gray-200" 
                                                    ...
                                                    <div className="md:col-span-5">
                                                        ...
                                                        <select className="w-full border-gray-300 rounded-md shadow-sm disabled:bg-gray-200"
                                                    ...
                                                    <div className="mt-6 pt-6 border-t border-dashed border-gray-300">
                                                        ...
                                                        <div className="space-y-3">
                                                            {field.options && field.options.map((opt, idx) => (
                                                                <div key={`opt-${field.field_id}-${idx}`} className="flex items-center space-x-2">
                                                                    <input type="text" className="flex-1 border-gray-300 rounded-md shadow-sm disabled:bg-gray-200"
                                                        ...
                                                        {isLocked && (
                                                            <p className="mt-3 text-sm text-green-600 font-medium">
                                                                <i className="fa-solid fa-link mr-1"></i> Auto-synced for this department.
                                                            </p>
                                                        )}
                                                    </div>
                                                )}
